...
class WXBizMsgCrypt
{
     ...
     */
    public function encryptMsg(string $replyMsg, string $token, string $appid, string $key): int|array
    {
        $timeStamp = time();
        $nonce     = $this->getRandomStr(8);

        $encrypt = $this->encrypt($replyMsg, $appid, $key);
        if (is_int($encrypt)) {
            return $encrypt;
        }
     ...
     * @return int|string
     */
    public function encrypt(string $text, string $appid, string $key): int|string
    {
        try {
     ...
     */
    public function decryptMsg(array $post, array $get, string $appid, string $key, string $token): bool|int|string
    {
//        验证key的长度有效性
        if (strlen($key) != 43) {
...
//      查看小xml的加密数据是否得到
        ...
        return $this->decrypt($post['Encrypt'], $appid, $key);
    }
     ...
     * @return int|string
     */
    public function decrypt(string $encrypted, string $appid, string $key): int|string
    {
        $key = base64_decode($key . '=');
        try {
            ...
            return $from_app_id != $appid ? self::VALIDATE_APP_ID_ERROR : $xml_content;
     ...
     */
    protected function PKCS7Decode(string $text): string
    {
     ...
     */
    protected function PKCS7Encode(string $text): string
    {
     ...
     * @return int|string
     */
    public function getSHA1(string $token, string $timestamp, string $nonce, string $encrypt_msg): int|string
    {
        //排序
        try {
     ...
     */
    protected function getRandomStr(int $length = 16): string
    {
        $str     = "";
